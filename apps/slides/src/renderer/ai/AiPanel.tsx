...
export function AiPanel({
  ...
  return (
    ...
    >
      ...
      {showSettingsModal && <AiPanelApiSettingsModal onClose={() => setShowSettingsModal(false)} />}
    </aside>
  )
}

function AiPanelApiSettingsModal({ onClose }: { onClose: () => void }) {
  ...
  useEffect(() => {
    void window.slidesApi?.getAiSettings?.().then((settings: any) => {
      if (settings?.providers?.custom) {
        setApiKey(settings.providers.custom.apiKey || '')
        setModel(settings.providers.custom.model || '')
        setBaseUrl(settings.providers.custom.baseUrl || 'https://api.openai.com/v1')
      }
    })
  }, [])
  ...
  const fetchModels = async (url?: string, key?: string) => {
    ...
    setModels([])
    try {
      const modelsUrl = `${targetUrl.replace(/\/$/, '')}/models`
  ...
  const handleSave = async () => {
    ...
    await window.slidesApi?.setAiSettings?.(settings as any)
    setSaved(true)
    setTimeout(() => {
      onClose()
    }, 800)
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div style={{ display: 'flex', gap: 8, marginBottom: 16, flexWrap: 'wrap' }}>
          <button
            ...
            onClick={() => {
              ...
              setApiKey('ollama')
              void fetchModels(ollamaUrl, 'ollama')
            }}
          >
          ...
          <button
            ...
            onClick={() => {
              setBaseUrl('https://<your-remote-server>:11434/v1')
              setApiKey('ollama')
              setModels([])
              setModel('')
            }}
        ...
        {models.length > 0 && (
          <div style={{ fontSize: 11, color: '#4ade80', marginBottom: 12 }}>
